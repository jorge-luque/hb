package com.google.android.gms.internal.ads;

import android.view.View;
import com.google.android.gms.ads.internal.zze;

/* compiled from: com.google.android.gms:play-services-ads@@19.1.0 */
final class zzcsl implements zze {
    private final /* synthetic */ zzdgo zzggq;
    private final /* synthetic */ zzdha zzghe;
    private final /* synthetic */ zzbaj zzgih;
    private final /* synthetic */ zzcsr zzgii;
    private final /* synthetic */ zzcsj zzgij;

    zzcsl(zzcsj zzcsj, zzbaj zzbaj, zzdha zzdha, zzdgo zzdgo, zzcsr zzcsr) {
        this.zzgij = zzcsj;
        this.zzgih = zzbaj;
        this.zzghe = zzdha;
        this.zzggq = zzdgo;
        this.zzgii = zzcsr;
    }

    public final void zzh(View view) {
        this.zzgih.set(this.zzgij.zzgig.zza(this.zzghe, this.zzggq, view, this.zzgii));
    }

    public final void zzjy() {
    }

    public final void zzjz() {
    }
}
