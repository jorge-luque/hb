        ...
        if (!TextUtils.isEmpty(str)) {
            try {
                ...
                if (jSONObject.optInt("status", -1) != 1) {
                    this.zzduh = false;
                    zzazw.zzfc("App settings could not be fetched successfully.");
                    return;
                }
                this.zzduh = true;
                    ...
                    for (int i = 0; i < optJSONArray.length(); i++) {
                        ...
                        if (!TextUtils.isEmpty(optString)) {
                            if (!TextUtils.isEmpty(optString2)) {
                                if (AdMostInterstitial.ZONE_TYPE_INTERSTITIAL.equalsIgnoreCase(optString)) {
                                    this.zzdud.add(optString2);
                                } else if (AdMostInterstitial.ZONE_TYPE_REWARDED.equalsIgnoreCase(optString) && (optJSONObject = jSONObject2.optJSONObject("mediation_config")) != null) {
                ...
                zzazw.zzd("Exception occurred while processing app setting json", e);
